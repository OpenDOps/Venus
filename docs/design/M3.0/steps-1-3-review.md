...
- **The port is not reachable by untrusted callers.** [step-compose-hub](./plan.md#4-step-compose-hub) is what exposes it, and the auth module is deferred by design, so “later” for a security row means *once the port is reachable*.
...
| Medium/low done       | [L3](#l3--503-lies), [L5](#l5--drain-race), [L6](#l6--ghost-client), [L9](#l9--non-upgrade-get-is-400-not-health-json), [L10](#l10--heartbeat_many-aborts-on-the-first-sql-error), [L11](#l11--lease-ttl-is-only-2-heartbeats), [L12](#l12--serve-error-skips-shutdown), [L13](#l13--one-corrupt-trail-bin-bricks-a-room), [L14](#l14--notify_waiters-can-lose-the-stop-wakeup), [L15](#l15--malformed-frame-truncates-a-batch-silently) / [P1](#p1--attach-holds-clients-during-encode), [P2](#p2--export-holds-rooms-lock), [P3](#p3--compact-every-tick), [P4](#p4--sequential-insert), [P5](#p5--head-blob-loads-body), [P7](#p7--update-copied-per-persist-and-per-client), [P8](#p8--persist_tasks-grows-forever), [P9](#p9--cold-room-hydrate-stampede) |
| Resource exhaustion   | [S1](#s1--unbounded-outbound-queue-per-client) **done**; [S3](#s3--no-ws-message-cap-no-server-ping) **done**; [S5](#s5--runtime-image-is-the-toolchain-image-as-root) **done**; [S6](#s6--outbound_cap-bounds-frames-not-bytes) **done**; [S10](#s10--persist-buffer-is-unbounded) **done**; [S2](#s2--unauthenticated-room-creation-is-the-amplifier) id shape **done** (eviction is [P6](#p6--rooms-never-evicted) / step 7); [S7](#s7--unauthenticated-export-is-a-read-amplifier) crate slice **done** (closing the route is auth); [S9](#s9--cors-allows-any-method-from-six-localhost-origins) **done** |
...
| Third pass open        | [P13](#p13--dirty-trigger-is-per-row-with-a-subtransaction) (step 8). Closing [S7](#s7--unauthenticated-export-is-a-read-amplifier)’s route is auth. |
| Remaining             | [Still to fix](#still-to-fix)                                                                                                                                                                                                                                                                                         |
| Auth                  | Deferred — separate authorization module, not this crate’s merge buffer                                                                                                                                                                                                                                                                                                                                                                                      |
| Board next            | [step-compose-hub](./plan.md#4-step-compose-hub)                                                                                                                                                                                                                                                                                                                                                                                                             |
| Steps 4–9             | Not claimed. Compose / lease / dirty **code** exists as scaffolding                                                                                                                                                                                                                                                                                                                                                                                          |
...
Lease/drain completeness is [step-ha-owner](./plan.md#7-step-ha-owner), but the incomplete path already runs in `main.rs`. L4/L5 proposals are the crate shape step 7 will need; they can land before that step’s DoD.
...
| [L4](#l4--lease-steal-ram-stays)                          | Med  | open     | Lease steal, RAM stays                     | `lease.rs`, `Hub`                        | Heartbeat miss only logs. Old process keeps apply + persist. New owner hydrates a second `Doc`. Split-brain until the old process exits. Step 7 must shed the room. |
...
| [P6](#p6--rooms-never-evicted)                      | Rooms never evicted                                                                                                                       | `Hub.rooms`                            | Empty wiki keeps `Doc`, persist task, and lease heartbeat forever.                                                          |
...
| [P8](#p8--persist_tasks-grows-forever)              | **done** — persist `JoinHandle` on the `Room`                                                                                             | `Room.persist_task`                    | Was: `Hub.persist_tasks` Vec of every room ever opened. Idle rooms still keep the task until [P6](#p6--rooms-never-evicted). |
...
| [P13](#p13--dirty-trigger-is-per-row-with-a-subtransaction) | Low → Med | Small | Dirty trigger is per row with a subtransaction | `schema.sql` | [P4](#p4--sequential-insert) made the flush one statement, but N bins still cost N trigger calls, N plpgsql `EXCEPTION` savepoints, and N upserts to the same `dirty` row. Past 64 the subxid cache overflows. |
...
**Landed** (2026-09-12): `Room.persist_task` holds the `JoinHandle`. `spawn_persist` stores it while `Hub.rooms` is still locked so shutdown cannot miss it. `shutdown` / `abort_persist_no_flush` take the handle off each live room. No `Hub.persist_tasks` Vec. Idle rooms still keep the task until [P6](#p6--rooms-never-evicted). Test: `concurrent_get_room_single_flight_per_workspace` (handle present after `get_room`; gone after abort).
...
| [S2](#s2--unauthenticated-room-creation-is-the-amplifier)          | **done** (id shape) — 1..=64 ASCII slug at the route; eviction is [P6](#p6--rooms-never-evicted) | `collaboration_get`, blob, export | Was: any random `workspace_id` took a lease, hydrated, and spawned an immortal persist task. Garbage ids are now 400 before lease or SQL. A *valid* unknown id still opens a room until P6. |
| [S3](#s3--no-ws-message-cap-no-server-ping)                        | **done** — 4 MiB cap; ping 30s / pong wait 10s                   | `collaboration_get`, `handle_socket` | Was: 64 MiB default, no server ping; half-open sockets kept their `clients` entry.                                                                                                                                           |
...
| ID | Sev | Effort | Issue | Where | Effect |
| --- | --- | --- | --- | --- | --- |
| [S6](#s6--outbound_cap-bounds-frames-not-bytes) | **done** (Med → High) | Small–med | `OUTBOUND_CAP` bounded frames, not bytes | `Room`, `handle_socket` | Was: 256 slots × up to 4 MiB (`WS_MAX_MESSAGE`) ~1 GiB retained per stalled reader. Now an 8 MiB queued-byte budget; detach on over-budget the same as Full. |
...
**Landed** (2026-09-13): id-shape slice only. `workspace_id_ok`: 1..=64 bytes, first character ASCII alphanumeric, rest alphanumeric / `-` / `_` / `.`. Applied on every `{workspace_id}` route (health, WS upgrade, export, blobs) **before** `get_room` / SQL. 400 `{ "error": "invalid workspace_id" }` — the raw id is not echoed. Does not refuse unknown-but-well-shaped ids. Tests: `http::s2::workspace_id_shape`; `bad_workspace_id_is_400_and_does_not_acquire` (no lease); `bad_workspace_id_blob_and_export_are_400` (no blob row). Eviction remains [P6](#p6--rooms-never-evicted).
...
### Ahead of the board (not step 4 / 7 close-out)

`docker-compose.yml` already has `postgres` + `hub` + `web` and an HA profile `hub-b`. Schema already has `workspace_lease` and the dirty trigger. Useful scaffolding.

- Step 4 still needs Compose restart-without-volume evidence.
- Step 7 still needs a second process that refuses and sheds on steal ([L4](#l4--lease-steal-ram-stays)). SIGTERM join/`drop_all` is [L5](#l5--drain-race) **done**.
- Step 8 still needs dirty as the observer contract, not “table exists”.
...
8. [L5](#l5--drain-race) join persist tasks. **done**. Then [L4](#l4--lease-steal-ram-stays) / [P6](#p6--rooms-never-evicted) shed (step 7 DoD).
...
7. [P7](#p7--update-copied-per-persist-and-per-client) `Bytes` fan-out. **done**. [P8](#p8--persist_tasks-grows-forever) / [P9](#p9--cold-room-hydrate-stampede) **done** (handle on `Room`; single-flight per `workspace_id`). Idle eviction remains [P6](#p6--rooms-never-evicted). ([P4](#p4--sequential-insert) batch `INSERT` is **done**.)
8. [S5](#s5--runtime-image-is-the-toolchain-image-as-root) slim non-root image. **done**. [S2](#s2--unauthenticated-room-creation-is-the-amplifier) id shape **done**. Remaining S2 eviction / S4 quota: [Still to fix](#still-to-fix).
...
8. [P13](#p13--dirty-trigger-is-per-row-with-a-subtransaction) statement-level dirty trigger — land with [step-8](./plan.md#8-step-dirty). [S7](#s7--unauthenticated-export-is-a-read-amplifier) export single-flight and blob cache headers **done**. Closing the route is auth.

## Still to fix

Every first- and second-pass fix is landed, plus third-pass [L16](#l16--hydrate-failure-squats-the-lease)–[L21](#l21--fan-out-is-lost-if-frame-encode-fails-after-persist) / [S6](#s6--outbound_cap-bounds-frames-not-bytes) / [S8](#s8--sslmodedisable-is-hardcoded) / [S10](#s10--persist-buffer-is-unbounded) / [P10](#p10--mutexdoc-serializes-reads-against-apply)–[P12](#p12--heartbeat-is-n-round-trips-and-bursts) / [P14](#p14--duplicate-index-on-crdt_update)–[P16](#p16--blob_post-copies-the-body) / [S7](#s7--unauthenticated-export-is-a-read-amplifier) crate slice / [P17](#p17--pool-defaults-are-the-only-pool-config) / [S9](#s9--cors-allows-any-method-from-six-localhost-origins). [S2](#s2--unauthenticated-room-creation-is-the-amplifier) id shape and the cleanup row (`chrono`, `compact_after < 1`, `abort_persist_no_flush`) are **done**. Nothing is left in this crate except [P13](#p13--dirty-trigger-is-per-row-with-a-subtransaction) with [step-8](./plan.md#8-step-dirty). Closing [S7](#s7--unauthenticated-export-is-a-read-amplifier)’s route is auth, not this crate. Do not reopen steps 1–3. Do not invent plan scenarios. Auth stays a separate module. Board next is [step-compose-hub](./plan.md#4-step-compose-hub); that step does not wait on [L4](#l4--lease-steal-ram-stays) / [P6](#p6--rooms-never-evicted).

### This crate (before the Compose port is the product path)

Crate config / CORS work is **done**. Remaining items are step 7, auth, or [P13](#p13--dirty-trigger-is-per-row-with-a-subtransaction) with step 8.

### [step-ha-owner](./plan.md#7-step-ha-owner) (step 7)

Same shed path. Prove with two processes; landing the crate shape earlier is allowed, the DoD is step 7.

| ID | Fix |
| --- | --- |
| [L4](#l4--lease-steal-ram-stays) | Heartbeat miss sheds the RAM room (`request_stop`, flush, `rooms.remove`, clients close). Do not `try_acquire` from shed. Old hub must stop `INSERT`. |
| [P6](#p6--rooms-never-evicted) | Idle eviction: no clients, empty persist buffer, idle ≥ TTL → same shed + `lease.drop_one`. |
| [S2](#s2--unauthenticated-room-creation-is-the-amplifier) remainder | P6 is the unbounded-room backstop until auth exists. |
...
- [Step 4](./plan.md#4-step-compose-hub): Compose restart-without-volume evidence. [P17](#p17--pool-defaults-are-the-only-pool-config), [S8](#s8--sslmodedisable-is-hardcoded), and [S9](#s9--cors-allows-any-method-from-six-localhost-origins) knobs are already in Compose.
- [Step 8](./plan.md#8-step-dirty): dirty as the observer contract, not “table exists”. [P13](#p13--dirty-trigger-is-per-row-with-a-subtransaction)’s statement-level trigger lands with it.
