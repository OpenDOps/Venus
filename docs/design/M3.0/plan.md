...
As an implementer I need the M1/M2 page to **keep syncing** after we **remove OctoBase keck**: a Venus-owned **Rust + y-octo hub** applies Yjs, broadcasts to other tabs, and persists to **Postgres tables we own**. Sockets for one wiki stick on `workspace_id`. Refresh and a second client still see the same blocks (including one image). Export still returns update v1. Live collab still does not wait on markdown or git. Venus still has no `wiki/` writer (that is M3).
...
6. HTTP **doc export** (`GET` of Yjs update v1 for the M0 workspace UUID). Current tree, not markdown, not `T0`. Reads Venus **SQL** (and/or live encode — Actual in api-map).
7. `workspace_lease`: a second hub process does not become a second live owner for the M0 workspace UUID while the first holds the lease. SIGTERM flushes persist then drops the lease.
...
11. Hub source is **Venus Rust + y-octo** (MIT/Apache on new files). Not an OctoBase fork. Not `jwst-rpc` linked in. Not a Node product `hub`. [licensing.md](../../legal/licensing.md).
...
| Convert / `fromDoc` / `toDoc` inside the hub                 | MDGate stays in the host (pane) and the **Rust convert worker** (pin / apply)         |
| Dual-write keck + hub                                        | Cut over; re-seed the M0 wiki if the volume is new                                     |
...
4. **One apply queue per room**, one persist buffer per room — **not** per socket. Owner / sticky grain is `workspace_id`, not `doc_id`.
5. **One collection, one page** still: workspace UUID `77e4a2b1-8b40-5979-a73c-fd4477216d00` (v5 of `venus-m0`) / BlockSuite `doc:home` (SQL `doc_id` is v5 of that name). Do not add catalog spaces.
6. **Pin** `yjs` **13.6.32.** Product hub apply is **y-octo** (same update v1). Browser Store stays that JS pin. Node `Y.applyUpdate` is recon-only.
...
Venus/
  ...
  crates/venus-hub/                  # product: Rust + y-octo
    # apps/hub/                      # recon-only Node spike; do not ship as Compose hub
  apps/web/
    src/host/providers/
...
SyncProvider (kind venus | octobase alias)
        ...
        ▼
Venus hub  (Compose service `hub`)  :3000  /collaboration/77e4a2b1-8b40-5979-a73c-fd4477216d00
...
| Apply        | **y-octo** `apply_update_from_binary_v1` (Rust). Product hub lives in `crates/venus-hub`. Node `Y.applyUpdate` is recon-only and must not ship. |
| Database     | **Postgres 16**, Compose `postgres`. Venus tables above. Not SQLite. Not `jwst` docs schema.                                                    |
| Routing      | **Wiki sticky:** `workspace_id` lease or hash. Not cookie/IP. Not `doc_id`.                                                                     |
| JS client    | Same framing as `OctoBaseKeckProvider`. New class optional. `new WebSocket(url, ['AFFiNE'])`.                                                   |
| Out of scope | OctoBase keck, AFFiNE Cloud, nbstore, Socket.IO, `@affine/core`, `toDoc`/`fromDoc` in the hub                                                   |
...
**Adds:** a decision and a map, not product UI. You know the **Rust** binary to run, that it speaks `AFFiNE` + y-protocols, where rows live, and that OctoBase is not linked.

Product apply is **Rust + y-octo**. A Node spike may prove the handshake; it must not become Compose `hub`. Write Actuals in api-map. Do not ship two hubs.

#### Work

1. Spike (throwaway ok): two `Y.Doc`s, `y-protocols` Step1/Step2/Update, **y-octo** apply on the server side **without** keck. Log bytes. Prove bold/`Format` apply does not throw.
2. Decide **kind** (`venus`) and **Chosen backend** in [api-map.md](../api-map.md): listen address, WS URL, export path (keep `GET /api/block/77e4a2b1-8b40-5979-a73c-fd4477216d00/export` as an alias if that keeps M1 tests), blob paths, DSN, volume name.
...
- Start the React two-tab UI here (spike the Rust hub).
- Implement git or `fromDoc` / `toDoc` in the hub.
- Ship Node `Y.applyUpdate` as Compose `hub`.
...
2. **Spike syncs**
  ...
  - **How:** `cargo test -p venus-hub --test recon` (in-process two `Room` clients; no keck, no Postgres). Fail if the spike still talks to keck.
...
6. Hard crash (no drain) may lose the unflushed persist batch (~1s). A client that still holds that update in its local `Y.Doc` resends it on reconnect (y-protocols). Not Redis. Reload / empty client hydrates from SQL only.
...
- Redis as a second CRDT / unflushed cache (client resend is the recovery for crash-before-flush).
...
4. **Crash, client resend**
  - **Given** hub + Postgres; A wrote `spike.k=v` over `AFFiNE` WS; persist has **not** flushed (do not wait ≥2s; do not SIGTERM drain).
  - **When** the hub process dies (RAM gone, Postgres stays). A still holds the update in its local doc and reconnects.
  - **Then** hydrate from SQL does **not** have the spike; after A’s Update, a new client / Step2 sees `spike.k=v`.
  - **How:** `cargo test -p venus-hub --test ws crash_unflushed_client_resend_restores`. Fail if recovery needs Redis. Fail if you waited for the persist tick instead of the client.
...
4. Health: `POST /collaboration/77e4a2b1-8b40-5979-a73c-fd4477216d00` → `{"protocol":"AFFiNE"}`.
...
2. BlobSource still `POST/GET /api/blobs/77e4a2b1-8b40-5979-a73c-fd4477216d00` (hub implements those paths).
...
- `doc_id` **sticky** as the owner algorithm (wiki is the shard key).
- Redis as a Y.Doc.
- Two owners “for HA.”
