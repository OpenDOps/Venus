...
**M3.0 board:** [step-recon-hub](../../M3.0/plan.md#1-step-recon-hub) and [step-store](../../M3.0/plan.md#2-step-store) are done. WS / Compose / lease / dirty in this folder describe the crate as it is; they are not milestone close-out.
...
| [architecture.md](./architecture.md) | Software architecture of the process |
| [files.md](./files.md) | Crate tree and what each `.rs` does |

Live CRDT HA: [M3.0/high-availability.md](../../M3.0/high-availability.md). Plan: [M3.0/plan.md](../../M3.0/plan.md). Dataflow of Venus: [architecture.md](../../architecture.md). Actuals: [api-map.md](../../api-map.md).
...
Set **either** a full DSN **or** the discrete parts (Compose uses the parts):

| Variable | Required | Compose default |
|---|---|---|
| `POSTGRES_HOST` | yes, unless `DATABASE_URL` | `postgres` |
| `POSTGRES_USER` | yes, unless `DATABASE_URL` | `venus` |
...
| `DATABASE_URL` | no | if set and non-empty, **wins** over the parts (not rewritten) |

`DATABASE_URL` example: `postgres://venus:venus@postgres:5432/venus?sslmode=disable`. Special characters in user/password are percent-encoded when the URL is built from parts. Startup logs `pg_sslmode`, never the DSN.
...
| `HUB_OWNER` | Lease id. Default `{HOSTNAME}-{pid}`. Set a distinct value on a second process (`hub-b` uses `hub-b`). |
...
```bash
docker compose --profile ha up --build hub-b
# hub-b publishes :3001
# WS to :3001/collaboration/77e4a2b1-8b40-5979-a73c-fd4477216d00 → 503 while hub holds the lease
```
...
Schema is applied at process start (`crates/venus-hub/src/schema.sql`) under a session advisory lock. Trigger `DROP`/`CREATE` runs only when a dirty trigger is missing so a second hub boot does not take AccessExclusive on live tables.
...
- **Flush:** append update binaries. `dirty.clock` is that `seq` (or snapshot `clock` on compact).
- **Compact:** background. New updates with `seq` greater than the compacted max are left in the trail.
- **Dirty:** `AFTER INSERT` on `crdt_update` and `AFTER INSERT OR UPDATE` on `crdt_snapshot` upsert `dirty`. The function catches `undefined_table` so a missing `dirty` table cannot roll back persist. The hub process does **not** `INSERT` into `jobs` (M3 observer).
