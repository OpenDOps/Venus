...
| [M3/plan.md](../M3/plan.md) | Thin implementation: one page, in-process idle/Flush, idle GET (or replica), `venus-sidecar`, one nested `wiki/`. |
...
A best-effort cut: sequential pins of dirty docs, then convert. Not a multi-space SQL transaction. Catalog pin in the same collect phase so `gitPath` matches the files you write. M3 has one page and a constant `gitPath` (`spec/home.md` unless api-map Actual differs).
...
| When | Source | Notes |
|---|---|---|
| **M3 idle / Flush** ([plan](../M3/plan.md) constraint 5) | Wait ≥ persist batch (**≥2s**), then `GET` hub **Export command** (api-map). Replica encode is allowed if recon records it as Actual. | Best-effort cut. Hidden AFFiNE client per page is **not** required to close M3. GET may trail live RAM by the persist window. |
| **M5 `T0` / flush-before-lease** | Replica encode, or wait ≥2s then GET | So Before is not missing in-memory-not-yet-SQL updates. |
| **Scale** | MVCC `SELECT` of dirty `crdt_*` rows after claim | [HA pin cut](./high-availability.md#pin-cut-lock-only-dirty-files-then-copy). Not the M3 mechanism. |
...
Product collab is Compose **`postgres` + `hub` (+ `web`)**. The hub **only** applies, broadcasts, persists `crdt_*` / blobs, and serves export. It does **not** offer “hold persist until pin copy finishes.” After persist, Postgres **upserts** `dirty(workspace_id, doc_id, clock)` (M3.0 trigger). The hub does not convert and does not write `jobs`.

M3 snapshotter (`venus-sidecar`) **reads** `dirty`, runs in-process idle + Flush (no `jobs` table in M3), pins, converts, commits. Convert is Path B: y-octo hydrate + M2 `from-doc.js` + **git2**. Not in `crates/venus-hub`. Not from the editor tab.
...
| [M3 plan](../M3/plan.md) | Thin-column implementation steps (recon started). |
...
Words: [glossary.md](../glossary.md). Dataflow: [architecture.md](../architecture.md). Hub process: [components/hub](../components/hub/).
