...
WYSIWYG still exists so PMs can write prose without Cursor. It is not the agent’s interface. Do not teach agents Yjs. Do not ship “copilot in the page” as the native path.
...
**Main feature** of the LLM wiki (with dual store and meaning-accept). Not a search index bolted on later.
...
This is what agents and PMs expand when they point at a span. It is not a second spec. Build: [agentic-binding](../design/Agents/agentic-binding.md) AB1 (spatial, after M3) then AB4 (temporal, after M6). Bound chat (AB2) and chat-edit (AB3) consume it.
...
- The hop between Venus and Cursor is **git**, not the clipboard. The runner kicks **Cursor CLI** (or SDK) to implement. **Cursor IDE is not required** for that loop. **CodeGraph CLI and/or Aider review that PR or branch** — they do not implement. **Select** the reviewer: [code-bind — select](../design/Agents/code-bind.md#select-codegraph-cli-or-aider-or-both).
...
**PMs and other managers are first-class.** They will not adopt an IDE. They adopt Venus: wiki, tree, WYSIWYG, **accept inbox**. That is their loop.

**The agentic loop does not require the Cursor IDE.** Venus **controls the implementer job**: pack at `wikiSha`, kick Cursor **CLI** (`agent -p` / SDK) on the product checkout, get a branch/PR, kick Aider/CodeGraph **review**, gates. Humans may still open the IDE to debug or type by hand. That is optional inner-loop chrome, not the product. If Venus *is* a Composer clone, Cursor already won. If Venus requires the IDE to participate, PMs have no heart and programmers are glued to a window they no longer need for the loop.
...
| **Managers never need Cursor** (IDE or CLI). WYSIWYG + accept inbox + tree is a complete path to write and bless intention. | Only programmers have a heart. |
...
| **Cursor CLI implements. Aider / CodeGraph CLI review the PR or branch.** The **IDE is optional.** | You sold a second IDE, or you glued the loop to a window nobody needs. |
...
| **Aider / CodeGraph CLI** | **Review only:** the agentic-loop **PR or branch** at `productSha` (graph/map + comments). **Not implementation.** Does not publish spec. **Select** before AB5. |
...
| | CodeGraph CLI | Aider |
|---|---|---|
| Job on the **PR / branch** | Queryable graph at `productSha` (callers, impact, later `implements`) | LLM **review comments** on that diff; packed repo map if CodeGraph is off |
| Implementation | **No** | **No** |

Neither replaces GitHub human review or meaning-accept. v1: always **run** the selected reviewer(s) after a branch/PR exists; flags are a **warning** (same softness as two-agent DoD). Later the step may fail. Merge check stays spec current or skip — not “Aider approved.”

Do not enrich every wiki request with the codebase. Do not delay M5 for this. After [M4](../design/venus-implementation-plan.md#m4--folder-tree--links--product-header-12-weeks) **record** the remotes; **select** the reviewer; the kick is on the runner ([AB5](../design/Agents/agentic-binding.md#ab5--code-bind--aider)).
...
- Step: `implement (Cursor CLI) → PR or branch → Aider / CodeGraph review (that PR/branch) → tests → breakpoint → done`. Runner kicks **external** CLIs. It does not write code. **Cursor CLI implements** (`agent -p` / SDK). **IDE is optional.** Aider / CodeGraph CLI **review** the product PR or branch ([code-bind](../design/Agents/code-bind.md#automated-review-after-implement)). **Select** the reviewer CLIs first.
...
- After Cursor CLI implements, kick Aider / CodeGraph CLI on that **PR or branch** — review only ([Workspace and Aider](#workspace-and-aider)).

Spatial + temporal graph is a **main feature**, not this gap list: [Multidimensional spec graph](#multidimensional-spec-graph). Two gits + analyzer is this gap: [Workspace and Aider](#workspace-and-aider).
...
| Aider or CodeGraph as **implementer** | Cursor writes the branch/PR. These CLIs **review** that PR or branch. Selling them as coding agents kills the loop. |
...
5. Plan runner **optional** (not every story). Two-agent DoD warn, then later refuse. Kick **Cursor CLI** to implement (IDE optional). **Aider / CodeGraph review the PR or branch**. Select reviewer CLIs **before** that kick.
