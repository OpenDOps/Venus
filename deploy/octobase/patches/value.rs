// Venus overlay on OctoBase 276e0e94719a652483119c5fea16be13293ee21c.
// Stock Content::Format => unimplemented!() in Value::from; DocPublisher history
// stringifies every item and panicked on Y.Text marks (bold/link/color).
use std::fmt::Display;
...
pub enum Value {
    ...
    XMLHook(XMLHook),
    XMLText(XMLText),
    /// Y.Text mark span. Open: value set (true, url, color); close: Null.
...
impl From<&Content> for Value {
    fn from(value: &Content) -> Value {
        match value {
            ...
            Content::Format { key, value } => Value::Format {
                key: key.clone(),
                value: value.clone(),
            },
            // actually unreachable
...
impl From<Value> for Content {
    fn from(value: Value) -> Self {
        match value {
            ...
            Value::Format { key, value } => Content::Format { key, value },
        }
    }
}
...
impl Display for Value {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Value::Format { key, value } => write!(f, "format({}={})", key, value),
            _ => write!(f, ""),
        }
    }
}
...
impl serde::Serialize for Value {
        ...
        S: serde::Serializer,
    {
        match self {
            ...
            Self::Format { key, value } => (key, value).serialize(serializer),
...
mod tests {
    ...
    #[test]
    fn format_value_roundtrip() {
        let content = Content::Format {
            key: "bold".to_string(),
            value: Any::True,
        };
        let value = Value::from(&content);
        assert_eq!(value.to_string(), "format(bold=true)");
        let close = Content::Format {
            key: "color".to_string(),
            value: Any::Null,
        };
        assert_eq!(Value::from(&close).to_string(), "format(color=null)");
        assert_eq!(Content::from(value), content);
    }
}
